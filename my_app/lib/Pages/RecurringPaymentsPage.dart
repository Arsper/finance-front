...
class _RecurringPaymentsPageState extends State<RecurringPaymentsPage> {
  ...
  void _openPaymentForm({Map<String, dynamic>? existing}) {
    ...
    final amountController = TextEditingController(
      text: existing != null
          ? (existing['amount'] as num).abs().toString()
          : "",
    );
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => Padding(
          ...
          child: SingleChildScrollView(
            child: Form(
              key: formKey, // Оборачиваем в Form
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(
                    existing == null ? "Новый шаблон" : "Редактирование",
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    onChanged: (val) =>
                        setModalState(() => selectedBillId = val),
                  ),
                  const SizedBox(height: 15),
                  ...
                  DropdownButtonFormField<String>(
                    initialValue: selectedPeriodicityKey,
                    decoration: const InputDecoration(
                      labelText: "Как часто?",
                      border: OutlineInputBorder(),
                      prefixIcon: Icon(Icons.repeat),
                    ),
                    items: periodicityMap.keys
                        .map((k) => DropdownMenuItem(value: k, child: Text(k)))
                        .toList(),
                    onChanged: (val) =>
                        setModalState(() => selectedPeriodicityKey = val!),
                  ),
                  const SizedBox(height: 15),
                    ...
                    onPressed: () async {
                      ...
                      if (formKey.currentState!.validate()) {
                        ...
                        bool ok = existing == null
                            ...
                            : await api.updateRecurringPayment(
                                existing['idPayment'],
                                data,
                              );

                        if (ok && context.mounted) {
                          // Используем context.mounted вместо просто mounted
      ...
      body: payments.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final symbol =
                    wallet['currencySymbol'] ?? wallet['currencyCode'] ?? '';
                      ...
                      children: [
                        ...
                        PopupMenuButton<String>(
                          onSelected: (val) async {
                            ...
                            if (val == 'delete') {
                              if (await api.deleteRecurringPayment(
                                p['idPayment'],
                              )) {
                                _loadData();
                              }
